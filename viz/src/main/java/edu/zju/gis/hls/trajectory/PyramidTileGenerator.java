...
import edu.zju.gis.hls.trajectory.analysis.index.quadtree.PyramidConfig;
...
import edu.zju.gis.hls.trajectory.analysis.rddLayer.EmptyGeometryLayer;
...
public class PyramidTileGenerator extends OperatorImpl {
  ...
  public JavaPairRDD<String, BufferedImage> generate(Layer layer) {
    // map 精简字段
      ...
      switch (feature.getGeometry().getGeometryType()) {
        case "Polygon": case "MultiPolygon":
          ...
          if (wkt.startsWith("POLYGON")) {
            ...
            nf = new Polygon((Polygon) feature);
            nf.addAttribute("wkt", wkt);
            polygonList.add((Polygon) nf);
          } else if (wkt.startsWith("MULTIPOLYGON")) {
            wkt = wkt.replace("MULTIPOLYGON (((", "").replace(")))", "");
            String[] polygons = wkt.split("\\)\\), \\(\\(");
            for (String polygon : polygons) {
              nf = new Polygon((Polygon) feature);
              nf.addAttribute("wkt", polygon);
              polygonList.add((Polygon) nf);
            }
          }
          // 处理多边形的内外环问题
          for (Polygon polygon : polygonList) {
            ...
            // 多边形外环必然存在
            nf = new Polygon((Polygon) feature);
            nf.setAttribute("wkt", rings[0]);
            result.add(new Tuple2<>(pairItem._1(), nf));
            // 多边形内环可能存在
            for (int i = 1; i < rings.length; i++) {
              nf = new Polygon((Polygon) feature);
              nf.setAttribute("color", config.getDefaultColor());
              nf.setAttribute("wkt", rings[i]);
              result.add(new Tuple2<>(pairItem._1(), nf));
            }
          }
          break;
        case "LineString": case "MultiLineString":
          if (wkt.startsWith("LINESTRING")) {
            ...
            nf = new Polyline((Polyline) feature);
            nf.addAttribute("wkt", wkt);
            result.add(new Tuple2<>(pairItem._1(), nf));
    ...
    Layer<String, Feature> layer5 = layer2.mapToLayer((Function<Tuple2<String, Feature>, Tuple2<String, Feature>>) pairItem -> {
      ...
      Feature feature = pairItem._2();
      Integer col = (Integer) feature.getAttribute("x"), row = (Integer) feature.getAttribute("y");
      Integer z = (Integer) feature.getAttribute("z");
      ...
      feature.setAttribute("wkt", strBuilder.toString());
      return new Tuple2<>(pairItem._1(), feature);
    });

    // 按照键值进行合并 实在想不出使用封装方法的意义何在 所以直接拆封装了
    JavaPairRDD<String, BufferedImage> resultPairRdd = layer5.groupByKey()
      .mapToPair(item -> {
  ...
  public <T extends KeyIndexedLayer> Layer run(T layer) {
    JavaPairRDD<String, BufferedImage> resultPairRdd = this.generate(layer.toLayer());
    imageWriter.write(resultPairRdd);
    return new EmptyGeometryLayer();
  }
