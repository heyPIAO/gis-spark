...
public class TWKBWriter
{
     ...
     */
    public TWKBWriter(int outputDimension, boolean includeSRID) {
     ...
     */
    public TWKBWriter(int outputDimension, int byteOrder) {
     ...
     * @param includeSRID indicates whether SRID should be written
     */
    public TWKBWriter(int outputDimension, int byteOrder, boolean includeSRID) {
    ...
    public void write(Geometry geom, OutStream os) throws IOException
    {
        if (geom instanceof Point) {
            if (geom instanceof TemporalPoint) {
                writeTemporalPoint((TemporalPoint) geom, os);
            } else {
            ...
            if (geom instanceof TemporalLineString) {
                writeTemporalLineString((TemporalLineString) geom, os);
            } else {
    ...
    private void writeTemporalPoint(TemporalPoint pt, OutStream os) throws IOException
    {
        if (pt.getCoordinateSequence().size() == 0)
            throw new IllegalArgumentException("Empty Temporal Points cannot be represented in TWKB");
        writeByteOrder(os);
        writeGeometryType(TWKBConstants.wkbTPoint, pt, os);
        writeTemporalCoordinateSequence(pt.getCoordinateSequence(), new long[]{pt.getInstant()}, false, os);
    }
    ...
    private void writeTemporalLineString(TemporalLineString line, OutStream os)
            throws IOException
    {
        writeByteOrder(os);
        writeGeometryType(TWKBConstants.wkbTLineString, line, os);
        writeTemporalCoordinateSequence(line.getCoordinateSequence(), line.getInstants(),true, os);
    }
    ...
    private void writeTemporalCoordinateSequence(CoordinateSequence seq, long[] timestamps, boolean writeSize, OutStream os)
            throws IOException
    {
        if (writeSize)
            writeInt(seq.size(), os);
        ...
        for (int i = 0; i < seq.size(); i++) {
            writeTemporalCoordinate(seq, timestamps[i], i, os);
        }
    }
    ...
    private void writeTemporalCoordinate(CoordinateSequence seq, long timestamps, int index, OutStream os)
            throws IOException
    {
        ByteOrderValues.putDouble(seq.getX(index), buf, byteOrder);
        os.write(buf, 8);
        ...
        ByteOrderValues.putLong(timestamps, buf, byteOrder);
        os.write(buf, 8);
    }
}
