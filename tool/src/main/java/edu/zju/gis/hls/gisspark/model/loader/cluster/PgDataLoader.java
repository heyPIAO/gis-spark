...
import edu.zju.gis.hls.trajectory.datastore.storage.helper.VoidSQLResultHandlerImpl;
 ...
 **/
@Slf4j
public class PgDataLoader extends DataLoader {
  ...
  protected void prepare() {
    ...
    pgHelper.runSQL(this.createTableSql(readerConfig, writerConfig), new VoidSQLResultHandlerImpl());
    log.info("DISTRIBUTE TARGET TABLE");
    pgHelper.runSQL(this.distributeTableSql(readerConfig, writerConfig), new VoidSQLResultHandlerImpl());
    pgHelper.close();
  }
  ...
  private String distributeTableSql(LayerReaderConfig rc, PgLayerWriterConfig wc) {
    ...
    return String.format("SELECT create_distribute_table('%s', '%s', '%s');",
  ...
  public static void main(String[] args) throws Exception {
    PgDataLoader pgLoader = new PgDataLoader(SparkSessionType.SHELL, args);
    pgLoader.exec();
  }
